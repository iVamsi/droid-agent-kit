...
# release.yml publishes the npm launcher with no explicit dist-tag, so every version claims
# `latest`. That is correct while pre-1.0 -- there is no stable release to shadow -- but wrong
# the moment one exists: the next prerelease published after it would overwrite `latest` and
# hide the stable version from `npm install`. Fail here so the first stable release forces the
# decision, then delete this block as part of adding the real dist-tag logic.
if [[ "$version" != *-* ]]; then
