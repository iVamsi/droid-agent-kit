...
- **Packaging decision:** `docs/adrs/0001-packaging.md` — thin npm launcher is the primary install path; MCPB (`distribution/mcp.json`) is secondary; OCI is rejected (no proven host adb/USB/socket passthrough). Registry metadata (`distribution/server.json`) is published only after clean-machine smoke tests pass.
...
- **Security model is local-only**: `ProcessRunner` is a generic, capability-agnostic executor that streams output to artifacts and redacts text; it no longer enforces the Gradle task allowlist. Gradle task authorization lives in a typed Gradle adapter (`SafetyConfig.isGradleTaskAllowed`, called before constructing the command). All output is passed through `Redactor` before being returned to agents.
