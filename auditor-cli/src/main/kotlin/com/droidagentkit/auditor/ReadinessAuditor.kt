...
class ReadinessAuditor(private val inspector: AndroidProjectInspector) {
    fun audit(root: Path, redactPublic: Boolean = false): ReadinessReport {
        val project = inspector.inspect(root)
        val risks = mutableListOf<ReadinessRisk>()
        var score = 0

        if (project.commandMatrix.any { it.id.contains("assemble") || it.id.contains("test") }) score += 20
        else risks += risk("missing-build-commands", Severity.ERROR, "Build commands are not discoverable", "No safe Gradle build or test command was inferred.", "Add a Gradle wrapper and document test/build tasks.")

        if (project.modules.any { it.hasUnitTests || it.hasAndroidTests }) score += 15
        else risks += risk("missing-tests", Severity.WARNING, "No test source sets were detected", "Expected src/test or src/androidTest in at least one module.", "Add tests or document why verification is manual.")
        ...
        else risks += risk("missing-agent-instructions", Severity.WARNING, "Agent instructions are missing", "No AGENTS.md, CLAUDE.md, GEMINI.md, Cursor, or Copilot instruction file was found.", "Generate AGENTS.md with droidagent audit --write-agents.")

        if (hasCi(root)) score += 10 else risks += risk("missing-ci", Severity.WARNING, "CI workflow was not detected", "No common CI workflow file was found.", "Add CI that runs the same commands listed in AGENTS.md.")

        if (documentsDeviceExpectations(root)) score += 10

        val secretEvidence = findPossibleSecrets(root)
        if (secretEvidence.isEmpty()) score += 10
        else risks += ReadinessRisk(
            id = "possible-secret",
    ...
    private fun risk(id: String, severity: Severity, title: String, evidence: String, fix: String) =
        ReadinessRisk(id, severity, title, listOf(evidence), fix)

    private fun hasCi(root: Path): Boolean = listOf(
        ".github/workflows",
        ".gitlab-ci.yml",
...
class AgentsDocumentGenerator {
    fun generate(report: ReadinessReport): String = buildString {
...
class AgentDocumentWriter {
    fun write(root: Path, report: ReadinessReport, mergeAgents: Boolean): List<ArtifactRef> {
